...
import os,time,json,logging,requests,base64,csv,sys,re
...
MAX_TRADES  = 8           # up from 5
SCAN_SECS   = 180         # 3 min — faster for scalping (was 5)
MIN_SCORE   = 65          # down from 75 — was blocking all trades
MIN_STRATS  = 3           # down from 4 — more opportunities
...
def ai_confirm(iid, direction, sigs, score, context):
    ...
    try:
        prompt=(f"Rate forex signal 0-100 for trade quality. "
                ...
                f"Strats:{','.join(sigs[:8])} | {context} "
                f"Reply JSON only: {{\"s\":75,\"r\":\"brief reason\"}}")
        r=requests.post(
            "https://api.anthropic.com/v1/messages",
            headers={"x-api-key":CLAUDE_API_KEY,"anthropic-version":"2023-06-01",
                     "content-type":"application/json"},
            json={"model":"claude-haiku-4-5-20251001","max_tokens":90,
        ...
        if r.ok:
            ...
            if m:
                ...
                reason=m.group(2)
                log.info("APEX-AI %s: base=%d ai=%d final=%d [%s]"%(iid,score,ai_s,final,reason))
                return final,reason
    except Exception as e: log.warning("APEX-AI error: "+str(e))
    return score,""
...
log.info("Strategies: 21 (15 classic + 6 SMC)")
log.info("APEX-AI: "+("ENABLED (Claude Haiku)" if CLAUDE_API_KEY else "DISABLED — set CLAUDE_API_KEY"))
log.info("MIN_SCORE=%d  MIN_STRATS=%d  MAX_TRADES=%d  SCAN=%ds"%(MIN_SCORE,MIN_STRATS,MAX_TRADES,SCAN_SECS))
log.info("="*55)

alert("APEX V4 ONLINE",
    "Hedge Fund Edition\n21 strategies (6 SMC) + Claude AI\nTarget: 10+ trades/day\nBriefings: 07:00/14:30/21:00 UK\nAI: "+("ON" if CLAUDE_API_KEY else "OFF — add CLAUDE_API_KEY env var"),
    "default","robot")
...
while True:
    try:
        ...
        save_state(state)
        w=state["wins"]; l=state["losses"]; t2=w+l
        log.info("%dW/%dL (%.0f%%) DL:%.2f Open:%d"%(w,l,w/t2*100 if t2 else 0,state["daily_loss"],len(state["trades"])))
        log.info("Next scan in %ds..."%(SCAN_SECS))
        consecutive_errors=0
